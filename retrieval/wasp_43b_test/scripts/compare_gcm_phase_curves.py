#!/usr/bin/env python3
"""Compare the prepared WASP-43b observations and the SWAMPE-JAX forward model
...
level), which removes the star + baseline to first order. SWAMPE-JAX curves are the
...
def swampe_model_curves(tau_pairs):
    ...
    for tr, td in tau_pairs:
        ...
        print(f"  SWAMPE-JAX tau_rad={tr:5.1f}h tau_drag={td:5.1f}h: "
...
def main() -> None:
    ...
    parser.add_argument("--no-model", action="store_true", help="skip the SWAMPE-JAX forward overlay")
    ...
    if not args.no_model:
        print("[running SWAMPE-JAX forward overlays]")
    ...
    for tr, td, ph, fp in model_curves:
        ...
        ax.plot(ph[order], fp[order], lw=2.0, ls="--",
                label=f"SWAMPE-JAX tau_rad={tr:g}h tau_drag={td:g}h")
    ...
    ax.set_title("WASP-43b 5-10.5 um: data vs Bell et al. 2024 GCMs vs SWAMPE-JAX forward model")
