...
def test_pilot_config_json_sane():
    ...
    assert cfg["Phibar"] == 4.0e6 and cfg["DPhieq"] == 3.5e6
    for flag in ("infer_tau_rad", "infer_tau_drag", "infer_planet_fpfs", "infer_noise_inflation"):
...
_X64_CHECK = textwrap.dedent(
    ...
    assert pipe.param_names == ["tau_rad_hours", "tau_drag_hours", "planet_fpfs", "noise_inflation"]
    ...
    th1 = np.array([10.0, 6.0, 0.003, 1.0]); th2 = np.array([10.0, 6.0, 0.003, 2.0])
    ...
    assert g.shape == (4,) and np.isfinite(g).all() and np.all(np.abs(g) > 0.0), f"bad grad {g}"
