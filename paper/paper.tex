...
\texttt{SWAMPE-JAX} was designed to fill this gap. It occupies the intermediate regime between fast one-dimensional retrieval models and computationally intensive three-dimensional GCMs, while retaining the complexity needed to interpret multidimensional atmospheres. \texttt{SWAMPE-JAX} is extremely fast (in order to enable broad grid searches). Furthermore, the differentiable \texttt{JAX} implementation further enables sensitivity analysis and is designed to support future gradient-informed Bayesian retrieval frameworks.
